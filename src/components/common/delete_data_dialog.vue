<template>
  <el-dialog v-model="propDelete.isShow" :title="propDelete.actionName" width="30%" draggable top="35vh" @close="fClose">
    ...
    <span style="{font-size: 13px}">	 本次将删除 <span class="hight-light">{{ propDelete.delCounts }} </span> 条数据, 确认删除吗 ？</span>
    <br/> <br/>
    <template v-if="propDelete.serverResCode===200">
      删除结果：
      <span class="hight-light">成功</span>
    </template>
    <template v-else-if="propDelete.serverResCode===400">
      删除结果：<span class="hight-light">失败 , {{ propDelete.serverResMsg }}</span>
    </template>
    <template v-else-if="propDelete.serverResCode!==0">
      <el-icon color="#ef2f2f">
        <WarningFilled/>
      </el-icon>
      删除操作出错：<span class="hight-light">{{ propDelete.serverResMsg }}</span>
    </template>
    <template #footer>
